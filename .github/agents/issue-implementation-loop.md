...
12. Merge the pull request into `main` only after all applicable tests/lints/checks pass, required code review has completed, no actionable review comments remain unresolved, and current repository policy allows the merge. Never queue auto-merge while Copilot review is requested but incomplete; if auto-merge is used after review completion, poll until the PR is actually merged before tagging or cleanup.
