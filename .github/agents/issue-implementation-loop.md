...
6. For this repository, treat only `cloud-ready` issues covering documentation, repository policy/automation, and other changes verifiable in the cloud as ready. Off-ramp issues that require Misty hardware, Foundry Local, Windows-only audio/SAPI5 behavior, product decisions, unclear criteria, or unavailable required verification.
