...
9. Monitor the pull request until required checks, required human review, and Copilot review are complete. Address actionable review feedback on the same branch, re-run verification, and re-request review when needed.
...
11. Merge the pull request into `main` only after successful verification, required approvals, no unresolved review comments, and repository policy allow the merge; otherwise report the exact blocker.
