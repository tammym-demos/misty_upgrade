...
- For autonomous issue-implementation-loop work, require a PR, passing required status checks, Copilot code review, issue AI-usage closeout, main-branch PR metadata tag, and branch/worktree cleanup. Owner review is not required unless current branch protection or the user explicitly requires it.
...
- **Firmware**: v2.0.2.140 / robot OS 2.0.2.11660. Misty Robotics was acquired by Furhat Robotics; no further firmware updates expected. This is the final firmware. `POST /api/system/update` returns `false` (no OTA available). Bootloader is locked — no custom firmware flashing possible.
- **Snapdragon 410 ML degradation**: The sensory services chip's ML models are degrading over time. Face detection is completely dead (zero WebSocket events). Keyphrase detection fails after ~2 cycles. These are unfixable — proprietary Qualcomm SNPE models with no update path. See `docs/lessons-learned.md`.

## Key Conventions

- **Latency SLO**: p50 < 10s, p95 < 15s end-to-end (see #21). The orchestration service logs per-stage timing: `[Pipeline Xms] STT=X LLM=X TTS=X history=N fallback=F cached=C`. Measured breakdown: STT ~400ms, LLM ~1200ms, TTS ~2-3s (cached: 0ms). TTS scales linearly with response length — keep `max_tokens` moderate (currently 50).
...
- **TTS fallback chain**: Kokoro-ONNX is primary TTS (speed 1.2x). If unavailable, pyttsx3 (Windows SAPI5) is used as fallback. Both are lazily initialized and thread-safe (locked). The API response includes `"ttsFallback": true` when fallback is used and `"ttsCached": true` on cache hits.
- **TTS audio cache**: In-memory LRU cache (max 200 entries) maps text→WAV file. Movement acknowledgments are pre-warmed at startup in a background thread with pinned (non-evictable) entries. Cached files use stable `cached_<hash>.wav` names excluded from the response file cleanup. Cache stats are reported in `/api/diagnostics` under `tts.cache_entries`.
...
- **Face recognition (#16)**: Opt-in via `USE_FACE_RECOGNITION=true`. **Misty's on-chip face detection is dead** — the Snapdragon 410 ML pipeline produces zero events (see `docs/lessons-learned.md`). Implementation uses the companion laptop webcam with OpenCV + ONNX face embeddings instead. During each conversation turn, face recognition runs concurrently with audio recording to identify the speaker. The recognized name is passed as `speaker_name` form field to `/api/orchestrate`, which injects it into the LLM system prompt ("You are currently talking to {name}"). Persists across follow-up turns within a conversation; cleared on re-arm.
...
| #21 | End-to-end latency — TTS cache, Kokoro 1.2x speed, temperature 0.7, max_tokens 50 | Improved |
| #24 | LLM ignores brevity — max_tokens=50, temperature=0.7, post-LLM truncation to 35 words/3 sentences | Mitigated |
